...
class DroneController:
    ...
    def pid(self):  # PID algorithm
        ...
        try:
            if(abs(self.drone_whycon_pose_array.poses[0].position.z-22) > 0.75 or abs(self.drone_whycon_pose_array.poses[0].position.y - 0) > 0.75 or  abs(self.drone_whycon_pose_array.poses[0].position.x-0) >0.75):
                self.start_time = time.time()
            self.node.get_logger().info(str(time.time()-self.start_time))
            if(time.time()-self.start_time>self.duration):
                self.down=1
            if(self.down==1):
                self.set_points[2] = self.set_points[2] - (self.set_points[2]-27 )*self.rate
