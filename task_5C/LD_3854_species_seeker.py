...
class DroneController:
    def __init__(self, node):
        ...
        self.done=0
        self.clock = Clock()
        self.set_land_points=[7.486, 8, 25]
    ...
    def count_leds_in_mask(self,mask, original_image):
        ...
        for i,led_contour in enumerate(countours):
            ...
            if(abs(centroid_x-320)<100 and abs(centroid_y-240)<100):
                try:
    ...
    async def image_callback(self,image):
        ...
        for label in np.unique(labels):
            ...
            numPixels = cv2.countNonZero(labelMask)
            try:
                if numPixels > 100 and self.drone_whycon_pose_array.poses[0].position.z<24 and self.publish==0 and sum(value for key, value in alien_find.items())<3 :
                    self.find=1
                    try:
                    ...
                    mask = cv2.add(mask, labelMask)
                elif(sum(value for key, value in alien_find.items())>3):
                    self.back=1
            except:
                pass
        ...
        if(self.publish==1 and self.publishalien==1):
                ...
                if(sum(value for key, value in alien_find.items())>=3):
                    self.back=1
    ...
    def pid(self):  # PID algorithm
        ...
        try:
            ...
            if(self.approuch==0 and self.back != 1):
